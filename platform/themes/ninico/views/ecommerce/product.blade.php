@php
    Theme::asset()->usePath()->add('jquery-bar-rating-css', 'plugins/jquery-bar-rating/fontawesome-stars.css');
    Theme::asset()
        ->container('footer')
        ->usePath()
...
{{-- Product Page - White Background Section --}}
<section class="product-page-content light-theme-section" style="background-color: #ffffff !important; padding: 40px 0;">
    <div class="container">
        <div class="product-area row pb-25">
            ...
            <div class="col-lg-4 col-md-8">
                <div class="tpproduct-details__content">
                    <div class="tpproduct-details__tag-area d-flex align-items-center mb-5">
                        ...
                        @if ($product->productLabels->isNotEmpty())
                            @foreach ($product->productLabels as $label)
                                <span class="tpproduct-details__tag" {!! $label->css_styles !!}>{{ $label->name }}</span>
                            @endforeach
                        @endif
                        ...
                        @if (EcommerceHelper::isReviewEnabled())
                            ...
                            <a class="tpproduct-details__reviewers" href="{{ $product->url }}#reviews"
                                data-bb-toggle="scroll-to-review">{{ __(':count Reviews', ['count' => $product->reviews_count]) }}</a>
                        @endif
                    </div>
                        ...
                        <div class="theme-bg p-3 mb-4">
                            <div class="mb-3">
                                ...
                                <div class="tpdealcontact__count">
                                    <div class="tpdealcontact__countdown" data-countdown="{{ $flashSale->end_date }}">
                                    </div>
                                </div>
                            </div>
                            <div>
                    ...
                    <form class="cart-form" method="POST" action="{{ route('public.cart.add-to-cart') }}">
                        @csrf
                        <input id="hidden-product-id" name="id" type="hidden"
                        ...
                        <div class="tpproduct-details__count d-flex align-items-center flex-wrap gap-2 mb-3">
                            @if (EcommerceHelper::isCartEnabled())
                                ...
                                <div class="d-flex gap-2 tpproduct-details__cart">
                                    <button class="btn add-to-cart" name="add_to_cart" type="submit" value="1"
                                    ...
                                    @if (EcommerceHelper::isQuickBuyButtonEnabled())
                                        <button class="btn bg-dark buy-now" name="checkout" type="submit"
                                            value="1" @disabled($product->isOutOfStock())>
                                            {{ __('Buy Now') }}
                                        </button>
                                    @endif
                                </div>
                        ...
                        <div class="d-flex align-items-center gap-4 mb-4">
                            @if (EcommerceHelper::isWishlistEnabled())
                                <div>
                                    <a class="wishlist add-to-wishlist text-muted small" href="#"
                            ...
                            @if (EcommerceHelper::isCompareEnabled())
                                <div>
                                    <a class="add-to-compare text-muted small" href="#"
                    ...
                    @if ($product->brand->id)
                        <div class="tpproduct-details__information tpproduct-details__brand">
                            <p>{{ __('Brand:') }}</p><span class="meta-value"><a href="{{ $product->brand->url }}"
                    ...
                    @if ($product->categories->isNotEmpty())
                        <div class="tpproduct-details__information tpproduct-details__categories">
                            ...
                            @foreach ($product->categories as $category)
                                <span>
                                    <a
                                        href="{{ $category->url }}">{{ $category->name . (!$loop->last ? ',' : null) }}</a>
                                </span>
                            @endforeach
                        </div>
                    @endif
                    ...
                    <div class="col-lg-12">
                        <div class="tpproduct-details__navtab mb-60">
                            <div class="tpproduct-details__nav mb-30">
                                <ul class="nav nav-tabs pro-details-nav-btn" id="myTabs" role="tablist">
                                    <li class="nav-item" role="presentation">
                                        <button class="nav-links active" id="description-tab" data-bs-toggle="tab"
                                    ...
                                    @if (EcommerceHelper::isProductSpecificationEnabled() &&
                                        ...
                                        <li class="nav-item" role="presentation">
                                            <button class="nav-links" id="av-specification-tab" data-bs-toggle="tab"
                                                data-bs-target="#nav-specification" type="button" role="tab"
                                                aria-controls="nav-specification" aria-selected="true">
                                    ...
                                    @if (is_plugin_active('faq') && count($product->faq_items) > 0)
                                        <li class="nav-item" role="presentation">
                                            <button class="nav-links" id="faq-tab" data-bs-toggle="tab"
                            ...
                            <div class="tab-content tp-content-tab" id="myTabContent-2">
                                <div class="tab-para tab-pane fade show active" id="description" role="tabpanel"
                                ...
                                @endif
                                @if (is_plugin_active('faq') && count($product->faq_items) > 0)
                                    <div class="tab-para tab-pane fade" id="faq" role="tabpanel"
...
{{-- End Product Page White Background Section --}}

@php($relatedProducts = get_related_products($product, 5))

@if ($relatedProducts->isNotEmpty())
    {{-- Related Products Section - Matching Home Page Product Section Design --}}
    <section class="product-area related-products pt-30 pb-30 scroll-animate animate-fast dark-theme-section bg-black">
        <div class="container">
            <div class="row">
                <div class="col-md-4 col-12">
                    <div class="tpsection mb-40">
                        <h2 class="tpsection__title" style="color: #ffffff !important;">
                            {{ __('Related Products') }}
                        </h2>
                    </div>
                </div>
            ...
            <div class="row">
                <div class="col-12">
                    @include(Theme::getThemeNamespace('views.ecommerce.includes.product-grid-home'), [
