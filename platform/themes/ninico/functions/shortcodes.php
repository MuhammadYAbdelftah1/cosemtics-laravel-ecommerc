...
app()->booted(function (): void {
    ...
    add_shortcode('full-banner', __('Full Width Banner'), __('Add full width banner section'), function (ShortcodeCompiler $shortcode) {
        return Theme::partial('shortcodes.full-banner.index', compact('shortcode'));
    });
    ...
    add_shortcode('promo-banner', __('Promotional Banner'), __('Add promotional banner section'), function (ShortcodeCompiler $shortcode) {
        return Theme::partial('shortcodes.promo-banner.index', compact('shortcode'));
    });
    ...
    add_filter(SHORTCODE_REGISTER_CONTENT_IN_ADMIN, function (string $data, string $key, array $attributes) {
        ...
        $form = ShortcodeForm::createFromArray($attributes)
            ...
            ->add('style', 'customSelect', [
                'label' => __('Style'),
                'choices' => [
                    'left' => __('Content Left'),
                    'right' => __('Content Right'),
                    'center' => __('Content Center'),
                ],
            ]);
    ...
    // Image + Text Section Shortcode
    add_shortcode('image-text', __('Image + Text Section'), __('Add image with text section'), function (ShortcodeCompiler $shortcode) {
        return Theme::partial('shortcodes.image-text.index', compact('shortcode'));
    });
    ...
    add_shortcode('testimonials', __('Testimonials Carousel'), __('Add testimonials carousel section'), function (ShortcodeCompiler $shortcode) {
    ...
    add_filter(SHORTCODE_REGISTER_CONTENT_IN_ADMIN, function (string $data, string $key, array $attributes) {
        ...
        $form = ShortcodeForm::createFromArray($attributes)
            ...
            ->add('background_color', 'customSelect', [
                'label' => __('Background Color'),
                'choices' => [
                    'white' => __('White'),
                    'light' => __('Light Gray'),
    ...
    if (is_plugin_active('ecommerce')) {
        ...
        Shortcode::register(
            ...
            function (ShortcodeCompiler $shortcode) {
                ...
                if (! $brandIds) {
                    $brands = get_featured_brands((int) $shortcode->limit ?: 6, ['slugable'], ['products']);
                } else {
                ...
                return Theme::partial('shortcodes.product-brands.index', compact('shortcode', 'brands'));
            }
        );

        Shortcode::setAdminConfig('product-brands', function (array $attributes) {
            return ShortcodeForm::createFromArray($attributes)
                ->withLazyLoading()
                ->add('title', 'text', [
                    'label' => __('Title'),
                ...
                ->add('brand_ids', ShortcodeTagsField::class, [
                    'label' => __('Brands'),
                    'attr' => [
                        'placeholder' => __('Choose brands'),
                    ],
                    'choices' => \Botble\Ecommerce\Models\Brand::query()
                        ->wherePublished()
                        ->pluck('name', 'id')
                        ->all(),
                ])
